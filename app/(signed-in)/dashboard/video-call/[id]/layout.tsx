...
function Layout({children}:{children :React.ReactNode}) {
    ...
    useEffect(() => {
        if (!client || !id) return;
        setError(null);
        const streamCallInstance = client.call("default", id as string);


        const joinCall = async () => {
            try{
                await streamCallInstance.join({ create: true});
                setCall(streamCallInstance);
            }catch (error) {
        ...
        // clean up function
        return () => {
            if (streamCallInstance && streamCallInstance.state.callingState === CallingState.JOINED){
                streamCallInstance.leave().catch(console.error);
            }
        };
